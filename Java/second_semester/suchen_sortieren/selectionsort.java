package second_semester.suchen_sortieren;

import java.util.*;

public class selectionsort {

    public static int minPosAb(int[] zahlen, int pos) {
        int minPos = pos;
        for (int i = pos + 1; i < zahlen.length; i++) {
            if (zahlen[i] < zahlen[minPos]) { // suche das kleinste Element im Array
                minPos = i;
            }

        }
        return minPos;
    }

    public static void selectionSort(int[] zahlen) {
        for (int i = 0; i < zahlen.length - 1; i++) {
            int minPos = minPosAb(zahlen, i);
            if (minPos != i) {
                quicksort.vertauschen(zahlen, i, minPos);
            }
        }
    }

    // wenn man alles vermischen will:

    public static void unsortieren(int[] zahlen) {

        // erstelle Index aus Anzahl Elemente und vertausche, stelle index mit stelle i

        for (int i = 1; i < zahlen.length; i++) {
            int rndIndex = new Random().nextInt(zahlen.length);
            quicksort.vertauschen(zahlen, i, rndIndex);

        }

    }

    /**
     * Unsere Lieblingsmethode
     *
     * @param args die Kommandozeilenparameter
     */
    public static void main(String[] args) {
        int anzahl = 10;
        int[] a;

        a = bestArray(anzahl);
        System.out.println("aufsteigend geordnetes Array");
        printArray(a);
        System.out.println("unsortieren:");
        unsortieren(a);
        printArray(a);
        System.out.println();

        a = worstArray(anzahl);
        System.out.println("aufsteigend geordnetes Array");
        printArray(a);
        System.out.println("unsortieren:");
        unsortieren(a);
        printArray(a);
        System.out.println();
        System.out.println();

        a = randArray(anzahl);
        System.out.println("zufälliges Array");
        printArray(a);
        System.out.println("selectionSort:");

        selectionSort(a);
        printArray(a);

    }

    // Hilfsmethoden

    /**
     * Methode zum Ausgeben eines Arrays von int-Zahlen
     *
     * @param zahlen das auszugebende Array von int-Zahlen
     */
    public static void printArray(int[] zahlen) {
        System.out.println(Arrays.toString(zahlen));
    }

    /**
     * Methode zum Erzeugen eines aufsteigend sortierten Arrays von int-Zahlen
     *
     * @param n die Groesse des zu erzeugenden Arrays
     * @return das erzeugte Array {0, 1, ... , n-1}
     */
    public static int[] bestArray(int n) {
        int[] zahlen = new int[n];
        for (int i = 0; i < n; i++) {
            zahlen[i] = i;
        }
        return zahlen;
    }

    /**
     * Methode zum Erzeugen eines absteigend sortierten Arrays von int-Zahlen
     *
     * @param n die Groesse des zu erzeugenden Arrays
     * @return das erzeugte Array {n-1, n-2, ... , 0}
     */
    public static int[] worstArray(int n) {
        int[] zahlen = new int[n];
        for (int i = 0; i < n; i++) {
            zahlen[i] = n - 1 - i;
        }
        return zahlen;
    }

    /**
     * Methode zum Erzeugen eines Arrays von zuf�lligen int-Zahlen
     *
     * @param n die Groesse des zu erzeugenden Arrays
     * @return das erzeugte Array aus Zufallszahlen
     */
    public static int[] randArray(int n) {
        int[] zahlen = new int[n];
        for (int i = 0; i < n; i++) {
            zahlen[i] = (int) (Math.random() * n);
        }
        return zahlen;
    }
}